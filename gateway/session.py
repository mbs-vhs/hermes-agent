...
class SessionStore:
    ...
    def _get_or_create_session_impl(
        ...
        # Capture the prior session_id (if any) for the emit pipeline to consume.
        # Without this the auto-reset path closes a session locally with no
        # corresponding gateway-level session:end (#28746 / Cluster E).
        #
        # MERGE (v2026.7.20): upstream restructured this block to move recovery I/O
        # OUT of the lock (_needs_recover / _query_recoverable_session, with a
        # publish-if-not-already-populated race guard). That restructure is taken —
        # it is a real concurrency fix — and this capture is RE-APPLIED on top of it
        # rather than the old in-lock form being kept. Both was_auto_reset and
        # db_end_session_id are initialised upstream of here on upstream's path.

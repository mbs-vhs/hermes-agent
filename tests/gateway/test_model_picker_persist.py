...
@pytest.mark.xfail(
    strict=True,
    reason=(
        "fork policy (ADR-072, CLAWD-3009): a --global picker tap does not "
        "persist — GatewayRunner._govern_model_persist refuses the config.yaml "
        "write for the manifest-governed main slot, on every profile. Upstream "
        "added this case in v2026.7.20 assuming the write lands. strict=True so "
        "it flips RED, not silently green, if the policy is ever relaxed; see "
        "test_picker_tap_refuses_persist_manifest_governed for the fork's "
        "positive assertion of the same surface."
    ),
)

...
# ── Fork status (ADR-072 / CLAWD-3009) ─────────────────────────────────────
# This whole file arrived with the v2026.7.20 merge and every case drives a
# ``--global`` switch, i.e. a config.yaml persist of the MAIN model slot. This
# fork refuses that persist: ``GatewayRunner._govern_model_persist`` neutralises
# it because the main slot is declared in substrate-contract/roster.yaml
# provider_policy and generated into each profile's config.yaml, so a chat-issued
# switch would clobber the generated block (the same refusal is asserted
# positively in tests/gateway/test_model_picker_persist.py and
# tests/gateway/test_model_command_flat_string_config.py).
#
# The #25107 fix these cases guard is real and IS present in the merged tree —
# it simply cannot be observed through the persist path on this fork. Marked
# xfail(strict=True) rather than deleted, so that (a) the next upstream merge
# stays clean, and (b) they turn RED the moment main-slot persistence is
# re-enabled, at which point the guard becomes live again and the marker should
# be removed.
_ADR072_MAIN_SLOT_IS_REFUSED = pytest.mark.xfail(
    strict=True,
    reason=(
        "fork policy (ADR-072): /model --global does not persist the "
        "manifest-governed main slot, so a config.yaml assertion cannot fire"
    ),
)

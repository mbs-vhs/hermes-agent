...
def run_conversation(
    ...
    # is injected into the CURRENT USER MESSAGE (never the system prompt) to
    # preserve prompt-cache prefix stability. Empty when disabled/unset.
    #
    # Read BEFORE the prologue (it was read after, pre-v2026.7.20) because the
    # prologue now stamps the ``api_content`` sidecar — the exact API-bound
    # bytes, persisted so the next turn replays them verbatim. If the seed were
    # composed only at the ``api_messages`` build, the stamp and the wire would
    # disagree by exactly this block and every following turn would re-prefill
    # from the injection point. Only ``agent._shared_conversation_id`` is
    # needed, and that is set at agent construction, so the move is safe.
    _recent_seed_block = ""
    try:
    ...
    _ctx = build_turn_context(
        ...
        recent_seed_block=_recent_seed_block,
    )
        ...
        for idx, msg in enumerate(messages):
            ...
            if idx == current_turn_user_idx and msg.get("role") == "user":
                if isinstance(_api_content, str) and _api_content:
                    ...
                    # Same argument order as the stamp site, seed included, so
                    # the two paths cannot produce different bytes.

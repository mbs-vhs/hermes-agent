...
# Upstream's v2026.7.20 additions. They interleaved textually with the fork's
# ADR-072 refusal test above (shared _setup_isolated_home boilerplate), which is
# why this file conflicted. Both survive: the fork governs the ``--global`` arm,
# upstream pins the no-flag / ``--session`` arms that governance never touches.

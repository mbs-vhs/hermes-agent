...
def _resolve_lifecycle_pinned() -> bool:
    """Resolve the CLAWD-1376 badge-free pinned-message lifecycle opt-in from
    env HERMES_TELEGRAM_LIFECYCLE_PINNED or config.yaml
    display.platforms.telegram.lifecycle_pinned, defaulting to False.  Mirrors
    the gateway/run.py::_create_adapter() post-construction step that the v0.18
    merge dropped when Telegram moved to this plugin — the port carried
    _notifications_mode but not this flag (review B1 / CLAWD-1376).  Without it,
    the flag is un-settable and a gateway silently reverts to the CLAWD-1144
    fresh-DM-per-restart lifecycle (badge spam)."""
    raw = os.getenv("HERMES_TELEGRAM_LIFECYCLE_PINNED", "")
    if not raw:
        try:
            from gateway.config import load_gateway_config
            from gateway.run import cfg_get
            _gw_cfg = load_gateway_config()
            _lp = cfg_get(_gw_cfg, "display", "platforms", "telegram", "lifecycle_pinned")
            if _lp not in {None, ""}:
                raw = str(_lp)
        except Exception:
            pass
...
def _build_adapter(config):
    ...
    notification mode + lifecycle-pinned opt-in (preserving the gateway/run.py
    ...
    try:
        adapter._lifecycle_pinned = _resolve_lifecycle_pinned()
    except Exception:
        adapter._lifecycle_pinned = False
    return adapter

...
def compose_user_api_content(
    ...
    # Cross-surface recent-context seed (ADR-065 / CLAWD-1542 Phase S). Threaded
    # THROUGH this function rather than appended at one call site, because the
    # v2026.7.20 merge made this the single composition point: the prologue stamps
    # its output as api_content and conversation_loop sends the same output. A seed
    # appended at only one of those two places would break the prompt-cache
    # invariant (persisted sidecar drifting from the bytes on the wire) — which is
    # exactly what this function exists to prevent.
    if recent_seed:
        injections.append(recent_seed)
    ...
    *,
    ...
    if (
        ...
        _api_content = compose_user_api_content(
            ...
            ext_prefetch_cache,
            plugin_user_context,
            getattr(agent, "_recent_seed_block", "") or "",
        )

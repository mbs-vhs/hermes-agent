...
# New with the v2026.7.20 merge. The four ``--global`` cases below assert a
# config.yaml persist of the MAIN model slot; this fork refuses that write in
# ``cli.py`` (``_handle_model_switch`` / ``_apply_model_switch_result`` both
# print the manifest-governed notice instead of calling ``save_config_value``),
# because the main slot is generated from substrate-contract/roster.yaml
# provider_policy and a chat-issued switch would clobber it.
#
# ``test_session_only_switch_does_not_touch_config`` is NOT marked: it asserts
# the non-persisting arm, which the fork keeps intact, and it passes.
#
# xfail(strict=True) rather than deleted so the next upstream merge stays clean
# and these flip RED — not silently green — if main-slot persistence is ever
# re-enabled, at which point the #25106 guard becomes live and the marker goes.
_ADR072_MAIN_SLOT_IS_REFUSED = pytest.mark.xfail(
    strict=True,
    reason=(
        "fork policy (ADR-072): /model --global does not persist the "
        "manifest-governed main slot, so config.yaml is never written"
    ),
)

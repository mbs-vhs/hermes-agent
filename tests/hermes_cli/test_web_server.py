...
# ADR-072 main-slot governance vs upstream's v2026.7.20 test additions
# (CLAWD-3009).  Upstream shipped twelve `test_set_model_main_*` cases that all
# assume POST /api/model/set with scope="main" persists.  This fork refuses that
# write with HTTP 403 on purpose: the main slot is declared in
# substrate-contract/roster.yaml provider_policy and generated into each
# profile's config.yaml, and a web-UI write clobbers the generated block — the
# exact drift ADR-072 exists to end (see the guard in hermes_cli/web_server.py
# and the matching refusals in cli.py / gateway/slash_commands.py).
#
# They are marked xfail(strict=True), NOT deleted and NOT relaxed:
#   * strict means the day the fork stops refusing, these turn RED rather than
#     silently going green — so this is a tripwire on the policy, not a mute.
#   * keeping upstream's bodies verbatim keeps the next upstream merge clean.
# Whether the web UI should stay governed is a product decision; if it is ever
# relaxed, drop this marker and the twelve cases pass as upstream wrote them.
_ADR072_MAIN_SLOT_IS_REFUSED = pytest.mark.xfail(
    strict=True,
    reason=(
        "fork policy (ADR-072): POST /api/model/set scope='main' is refused "
        "with 403 because the main slot is manifest-governed"
    ),
)
...
class TestWebServerEndpoints:
    ...
    # NOTE (CLAWD-3388): upstream's `test_model_set_maps_unknown_vendor_to_
    # aggregator` was dropped here, as it already was on this fork's main. It
    # asserts POST /api/model/set scope="main" returns 200/ok=True, which the
    # ADR-072 guard in hermes_cli/web_server.py refuses with 403 — so it fails
    # outright rather than xfailing like the `test_set_model_main_*` cases
    # above. Restore it if the main slot ever stops being manifest-governed.

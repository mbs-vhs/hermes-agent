...
def compose_user_api_content(
    ...
    recent_seed_block: str = "",
) -> Optional[str]:
    """Compose the API-bound content of the current turn's user message.

    Sources: memory-manager prefetch, the fork's recent cross-surface context
    seed (ADR-065 / CLAWD-1542 Phase S), and ``pre_llm_call`` plugin context
    with target="user_message" (the default). All are appended to the *API
    copy* of the user message only — the stored content stays clean.

    ``recent_seed_block`` is fork-local: upstream has no equivalent, so it is
    an optional trailing parameter. It MUST be composed here rather than at
    the ``api_messages`` build, because the sidecar invariant below only holds
    if the prologue's stamp and the wire come out of the same call.
    ...
    # Ordering is load-bearing: prefetch → recent seed → plugin context is the
    # order the fork sent before the v2026.7.20 merge, and a reorder changes
    # the bytes (hence the cache prefix) for every existing session.
    if recent_seed_block:
        injections.append(recent_seed_block)

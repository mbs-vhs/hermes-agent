...
if [ "$NO_RESTART" = "0" ]; then
  ...
  [ "${#units[@]}" -gt 0 ] || die "no ai.hermes.gateway-*.service units found under this user manager. If the fleet has moved (it now runs per-user from /opt/hermes-agent), this script cannot deploy it — use --no-restart to advance ONLY the ~/.hermes checkout, and restart its consumers yourself. As of CLAWD-2803 the ~/.hermes venv's consumers are the two OAuth refresh timers, which are oneshots and pick up new code on their next firing."

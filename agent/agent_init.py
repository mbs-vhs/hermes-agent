            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        # v0.18 dropped conversation_id from the sync_turn ABC
                        # (now messages=), so thread the derived (person,agent)
                        # key to the provider via initialize instead — the
                        # provider self-sources it from here (CLAWD-1542 /
                        # CLAWD-2481). Empty (CLI/stranger) => provider no-ops.
                        if agent._shared_conversation_id:
                            _init_kwargs["conversation_id"] = agent._shared_conversation_id
                    except Exception:
                        pass

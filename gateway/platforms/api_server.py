...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def _http_route_table(self) -> List[tuple]:
        ...
        routes: List[tuple] = [
            ...
            # FORK-ONLY. Upstream has no clarify surface, so its route table
            # omits this; folded in here rather than left behind by the
            # v2026.7.20 swap from open-coded router.add_* calls to this table
            # (CLAWD-3009). Pinned by
            # tests/gateway/test_upstream_merge_720_api_server_contracts.py.
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        # MERGE (v0.18): took upstream's shared _concurrency_limited_response()
        # helper. REVIEW: verify it counts IN-FLIGHT runs (not retained result
        # streams) so a fire-and-forget client (agent-meeting-space bus) can't
        # wedge the gateway at 429 — the CLAWD-1923 fix this replaced.
        limited = self._concurrency_limited_response()
        if limited is not None:
            return limited
                ...
                def _run_sync():
                    ...
                    with self._profile_scope(request_profile):
                        try:
                            ...
                            # FORK-ONLY (clarify surface). Upstream has no
                            # clarify_gateway, so its rewrite of this block
                            # simply omits the pairing; re-indented into
                            # upstream's profile scope rather than dropped.
                            ...
                            try:
                                ...
                                # FORK-ONLY: pair the clarify registration
                                # above. Nested so an approval-unregister
                                # failure cannot leak the clarify listener,
                                # and vice versa.
                                try:
        ...
        for run_id in stale:
            ...
            if task_done:
                try:
                    ...
                    pass
                # FORK-ONLY: same teardown for the clarify listener.
                try:
                    from tools import clarify_gateway

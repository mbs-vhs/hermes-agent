...
def _persist_model_switch(result) -> None:
    ...
    #
    # ADR-072: this main-model persist helper is retained (upstream's atomic
    # key-write structure) but NOT invoked in _apply_model_switch — the manual
    # /model switch is session-only so it can't clobber the roster-generated
    # config. See the neutralized `if persist_global:` block below.
...
def _apply_model_switch(
    ...
    *,
    ...
    if persist_global:
        # ADR-072: session-only, no persist. Provider/model is manifest-governed;
        # the --global persist to config.yaml is refused so a manual /model switch
        # can't clobber the roster-generated config. Upstream would call
        # `_persist_model_switch(result)` here — that is deliberately NOT invoked.
        # The in-session swap above (agent.switch_model + session override) still
        # applies; surface the refusal as a warning.
        refusal = (
